...
{  
	...
	EN25QXX_CS=0;                            //使能器件   
	SPI_ReadWrite_Byte(EN25X_ReadStatusReg);    //发送读取状态寄存器命令    
	byte=SPI_ReadWrite_Byte(0Xff);             //读取一个字节  
	EN25QXX_CS=1;                            //取消片选  
...
{   
	EN25QXX_CS=0;                            //使能器件   
	SPI_ReadWrite_Byte(EN25X_WriteStatusReg);//发送写取状态寄存器命令    
	SPI_ReadWrite_Byte(sr);               	//写入一个字节  
	EN25QXX_CS=1;                            //取消片选     	      
}   
...
{
	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_WriteEnable);      //发送写使能  
	EN25QXX_CS=1;                            //取消片选     	      
}
...
{  
	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_WriteDisable);     //发送写禁止指令    
	EN25QXX_CS=1;                            //取消片选     	      
} 	
...
u16 EN25QXX_ReadID(void)
{
	u16 Temp = 0;	  
	EN25QXX_CS=0;				    
	SPI_ReadWrite_Byte(0x90);//发送读取ID命令	    
...
{ 
 	u16 i;   										    
	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_ReadData);         //发送读取命令   
    SPI_ReadWrite_Byte((u8)((ReadAddr)>>16));  //发送24bit地址    
    SPI_ReadWrite_Byte((u8)((ReadAddr)>>8));   
    SPI_ReadWrite_Byte((u8)ReadAddr);   
    for(i=0;i<NumByteToRead;i++)
	{ 
        pBuffer[i]=SPI_ReadWrite_Byte(0XFF);   //循环读数  
    }
	EN25QXX_CS=1;  				    	      
}    
...
void EN25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_PageProgram);      //发送写页命令   
    SPI_ReadWrite_Byte((u8)((WriteAddr)>>16)); //发送24bit地址    
    SPI_ReadWrite_Byte((u8)((WriteAddr)>>8));   
    SPI_ReadWrite_Byte((u8)WriteAddr);   
    for(i=0;i<NumByteToWrite;i++)SPI_ReadWrite_Byte(pBuffer[i]);//循环写数  
	EN25QXX_CS=1;                            //取消片选 
...
{                                   
    ...
    EN25QXX_Wait_Busy();   //等待写入结束
  	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_ChipErase);        //发送片擦除命令  
	EN25QXX_CS=1;                            //取消片选     	      
...
{  
    ...
    EN25QXX_Wait_Busy();   
  	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_SectorErase);      //发送扇区擦除指令 
    SPI_ReadWrite_Byte((u8)((Dst_Addr)>>16));  //发送24bit地址    
    SPI_ReadWrite_Byte((u8)((Dst_Addr)>>8));   
    SPI_ReadWrite_Byte((u8)Dst_Addr);  
	EN25QXX_CS=1;                            //取消片选     	      
...
{ 
  	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_PowerDown);        //发送掉电命令  
	EN25QXX_CS=1;                            //取消片选     	      
    delay_us(3);                               //等待TPD  
}   
...
{  
  	EN25QXX_CS=0;                            //使能器件   
    SPI_ReadWrite_Byte(EN25X_ReleasePowerDown);   //  send EN25X_PowerDown command 0xAB    
	EN25QXX_CS=1;                            //取消片选     	      
    delay_us(3);                               //等待TRES1
}   
